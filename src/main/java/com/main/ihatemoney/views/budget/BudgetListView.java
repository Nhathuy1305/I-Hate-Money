...
import com.main.ihatemoney.data.entity.report.CategoryTotal;
import com.main.ihatemoney.data.service.PfmService;
import com.main.ihatemoney.security.SecurityService;
import com.main.ihatemoney.views.MainLayout;
import com.main.ihatemoney.views.forms.BudgetForm;
...
public class BudgetListView extends Main implements HasComponents, HasStyle {
    ...
    private void configureBudgetCards() {
        ...
        List<Budget> budgets = service.getBudgetByUserId(userId);
        budgets.stream().forEach(budget -> {
            BudgetListViewCard card = new BudgetListViewCard(
                    ...
                    service.getSumExpensesInDateRange(budget.getStartDate(), budget.getEndDate(), Type.EXPENSE, userId)
            );
    ...
    private void applySortingToBudgets(List<BudgetListViewCard> budgetListViewCards, String sortOption) {
        if (sortOption.equals("Newest to oldest")) {
            ...
            budgetListViewCards.sort(Comparator.comparing(card -> card.getBudget().getStartDate()));
        } else if (sortOption.equals("← Start Date")) {
            budgetListViewCards.sort(Comparator.comparing(card -> card.getBudget().getStartDate()));
            Collections.reverse(budgetListViewCards);
        } else if (sortOption.equals("End Date →")) {
            budgetListViewCards.sort(Comparator.comparing(card -> card.getBudget().getEndDate()));
        } else if (sortOption.equals("← End Date")) {
            budgetListViewCards.sort(Comparator.comparing(card -> card.getBudget().getEndDate()));
            Collections.reverse(budgetListViewCards);
        }
            ...
            H3 totalExpenses = new H3("Total Expenses: " +
                    currencyFormat.format(
                            service.getSumExpensesInDateRange(
                                    budget.getStartDate(), budget.getEndDate(), Type.EXPENSE, budget.getUserId())));
    ...
    private Component getBudgetChart(Budget budget) {
        ...
        List<Object[]> expenseResults =
                service.sumTransactionsInDateRangeByCategory(userId, Type.EXPENSE, budget.getStartDate(), budget.getEndDate());
